...
def main():
    ...
    # 官方名單 PDF → 參賽名單。賽事在「抽籤完、還沒打完」那段期間 API 既沒有比分也沒有
    # 名次,整場一位選手都查不到,但名單早就掛在 documents 裡了(264311 羽霸盃 755 人、
    # lapgo-128 大佛盃 1,413 人)。目標是「沒比分沒名次卻有名單 PDF」的賽事,寫成條件
    # 而不是清單,新到這個狀態的賽事每個月會自己被接住。
    # **不歸在 mylivescore 底下** —— 名單有兩個來源(mylivescore 的「報名結果」與 LAPGO
    # 公告裡的「選手名單」),而且這一步本來就是掃全庫,只跑 lapgo 時同樣要跑。
    # 要在 dedupe 之前(dominates 會看 entries)、rebuild_index 之前("e":1 是那時算的)。
    if not only or only & {"mylivescore", "lapgo"}:
        ok, dt = run("parse_entry_pdf.py", ["--all", "--apply"],
                     "官方名單 PDF → 參賽名單 (parse_entry_pdf)")
        results.append(("官方名單PDF", ok, dt))
