...
否則底下的競賽組別與逐場比分會被推到看不見的地方。**這純粹是顯示層的收合,
...
- `scripts/parse_entry_pdf.py` — **官方名單 PDF → entries(source=signup)**,吃三種版面:
  ...
  **完全沒有欄名表頭那一列**(第一列就是資料),靠欄名判角色的邏輯整份跳過 ——
  ...
  這版**不可用座標切欄**:長隊名的 word 會延伸進姓名欄的 x 範圍(實測某群跨度 152pt),
  以 x 分群會把單位和姓名黏成一群。但它**有格線**(報名結果 PDF 沒有,那才要走座標),
  改用 `find_tables()`:實測 16 頁每頁都是乾淨的 1 張表 × 6 欄(`編號|隊名|姓名` × 2)。
  三個坑:(a) **一頁可以有好幾個組別**,第二個之後的標題在**表格內部**獨佔一列
  (`['40-49歲女雙【共6組】', None, ...]`),只認頁首標題會把後面幾組全算進第一組
  (實測「40-49歲男雙」抽到 26 vs 宣告 7);(b) 續頁不重印標題,組別沿用前頁,
  但要拿編號的組序交叉驗證,對不上就不沿用;(c) 守門分母是**隊數**不是人數,
  got 算不重複的編號數。實測 21 組 497 隊全部「抽到 = 宣告」。
  另外兩種:
  ...
  **表頭還可以疊成好幾列**(2026-09 修,全庫 10 份中招):「衣服尺寸1」「參照成績1
  (2025成大盃)」這種欄,欄名被拆到主表頭的上一行與下一行,主表頭那列裡根本沒有它們
  → 少一道 x 界線,置中的尺寸值(XL/2L)落進選手格,解出「陳士智XL」「2L侯靖思」。
  失敗方式是**靜默的**:734564 成大盃 2,234 個姓名只有 53 個對得上比分,程式不會報錯。
  判準是 y —— 堆疊欄名離主表頭不到一般行距的 0.7 倍(3.6pt vs 行距 6.9pt),
  而最近的資料列/組別標頭至少隔一整個行距。268439 更極端:主表頭自己就被拆成兩列。
  **同一格裡「已經出現過的尾巴」要跳過** —— 有些 PDF 把文字疊印兩次做假粗體,
  words 回兩份,接起來變成「張祐榮祐榮」這種不存在的人(285271/301516/808316 共 13 筆
  已寫進庫裡)。只比對「是不是已接內容的尾巴」,不可以用「整串是不是重複兩次」判斷 ——
  「叮咚叮咚」「哈哈哈哈」「菠蘿菠蘿」都是真的隊名。
...
- `matches[]` 為 API schedule 原樣:teamA/B=單位名(自由填寫,同校多種寫法,**不做正規化**,
  前端用子字串模糊比對 + 變體合併);**同校寫法由 `common.js unitKey()` 認**(2026-09):
  剝縣市前綴/行政區/分隊字母/學制尾巴後比 core,「四維國小」「中市四維」「臺中市北屯區四維國民小學」
  都是 core「四維」。搜尋頁據此多收(兩者不互相包含,子字串比對會漏);單位頁**預設合併**
  core 相同且縣市、學制不衝突者,但名稱本身既無縣市也無學制(「中正」)時不自動合併;scoreinfo[].memberA/B=選手名(雙打以 - 或 / 連接)。
...
- **`groups[].drawUrl` 不再外連(2026-09-23 使用者決定)**,但**欄位保留在資料裡**。
  ...
  已結束 40% 通 / 48% 404,進行中 4% 通 / 72% 404,**未開打 0% 通 / 84% 404**
  (籤表頁要等主辦產生才存在)→ 從未開打的賽事點進去必然全死。
  使用者的原則是「有建好的資料庫就不需要外連」。已結束的賽事我們有逐場比分,
  **籤表能給的比分全給了還多給比分**,所以移除外連不損失資訊。
  欄位留著是因為那些頁面印著「取N名」—— 推導永遠猜不到、目前只能靠官方 PDF 的那個資訊,
  這裡有 4,257 組的量(見「取 N 名只存在於官方文件」那條)。
- **從 `matches[]` 重建晉級樹是可行的,但目前刻意不做(2026-09-23 實測)**:
  以「上一輪贏家 = 下一輪某一方」串接,個人賽 194 組**串上+輪空可解釋中位數 96%**
  (83 組 100%),團體賽 7 組中位數 100% —— 但**團體賽只能以單位名串**,
  連 members 一起串會掉到 0%(每輪先發不同,同 `_entry_key` 的老坑)。
  仍有「上一輪輸了卻又出現」的矛盾案例(排名賽/敗部/來源自相矛盾)。
  不做的理由:逐場比分已含籤表的全部資訊且更多,晉級樹只是換個畫法,投入產出比低。
