...
1. **三來源增量抓取**(最後只重建一次索引):
   ...
   **各來源的判讀基準**(抓回的內容與現有檔案相同就不寫檔,所以「更新 N」都是真實變動):
   ...
   - **lapgo**:羽球賽事約 60~70 場,每月新增 1~5 場。
     ...
     更新行尾的 **「N 筆公告文件」** 是從賽事「最新消息」抓回來的官方文件
     (選手名單/抽籤結果/賽程,檔案掛 Google Drive)。**選手名單就是靠這條進來的** ——
     沒有這些文件,lapgo 的賽事在開打前一位選手都查不到。數字忽然歸零要查
     `getWebContent`/`getNewsContent` 是否改版。
     行尾的 **「籤表 N 組 M 人次」** 是抽籤結果 API(`getSessionMapData`)抓回來的分組籤表,
     只出現在還沒有比分的賽事,會**整份取代**該場的 PDF 選手名單(之後 parse_entry_pdf
     就不再讀那場的 PDF)。主辦公告「抽籤結果出爐」通常不附檔,只能靠這條。
     `[籤表] … 疑似還沒抽完,不收` 代表籤位不到官方隊數的 90%,下個月會自動再試。
     公告說已抽籤、卻一直沒有這行,要查 `getSessionGroup`/`getSessionMapData` 是否改版。
   ...
   - **官方名單 PDF → 參賽名單**(`parse_entry_pdf`):印「N 組 M 人 覆蓋 X%」。
     吃兩種版面 —— mylivescore 的「報名結果」與 LAPGO 公告裡的「選手名單」。
     覆蓋率是拿 PDF 自己宣告的「【共N組】」當分母算的,**應該是 100%**;
     低於 100% 或出現 `[組數對不上]` 就去看是哪一組,多半是版面變了。
     `不收:讀不到表頭` 代表那份是還沒支援的舊版面(目前 lapgo-19/27 兩場),不是失敗。
